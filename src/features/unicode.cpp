...
BOOL WINAPI utf8_text_metrics(HDC hdc, LPTEXTMETRICA lptm) {
  ...
  lptm->tmLastChar = static_cast<char>(wide.tmFirstChar);
  lptm->tmDefaultChar = static_cast<char>(wide.tmFirstChar);
  lptm->tmBreakChar = static_cast<char>(wide.tmFirstChar);
